...
import hummingbot.connector.exchange.peatio.peatio_http_utils as http_utils
from hummingbot.connector.exchange.peatio.peatio_api_order_book_data_source import (
    PeatioAPIOrderBookDataSource,
)
...
class PeatioExchange(ExchangeBase):
    """
    PeatioExchange connects with Peatio exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param peatio_api_key: The API key to connect to private Peatio APIs.
        ...
        self._set_order_book_tracker(PeatioOrderBookTracker(
            throttler=self._throttler,
            trading_pairs=trading_pairs))
        self._user_stream_tracker = PeatioUserStreamTracker(
            throttler=self._throttler,
    ...
    async def _api_request(self,
        ...
        parsed_response = await http_utils.api_call_with_retries(
            method=method,
            endpoint=endpoint,
            auth_headers=self._peatio_auth.get_headers if is_auth_required else None,
            params=params,
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> CancellationResult:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on Peatio)
    ...
    def stop_tracking_order_exceed_not_found_limit(self, tracked_order: PeatioInFlightOrder):
        """
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        return await PeatioAPIOrderBookDataSource.fetch_trading_pairs()

    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        # This method should be removed and instead we should implement _get_last_traded_price
        return await PeatioAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            throttler=self._throttler
        )
